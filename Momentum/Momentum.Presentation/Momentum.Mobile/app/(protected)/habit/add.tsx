...
import { useCreateHabit, useGetHabit, useUpdateHabit } from "@/api/habits/use-habits";
import { useAuth } from "@/lib/auth";

// Habit icons
const HABIT_ICONS = [
  "water-outline", "book-outline", "fitness-outline", "leaf-outline", "walk-outline",
  "bicycle-outline", "musical-notes-outline", "brush-outline", "camera-outline",
  "heart-outline", "restaurant-outline", "bed-outline", "sunny-outline", "moon-outline",
  "code-outline", "laptop-outline", "calculator-outline", "pencil-outline",
  "call-outline", "mail-outline", "gift-outline", "trophy-outline", "star-outline",
  "time-outline", "alarm-outline", "stopwatch-outline", "hourglass-outline"
];
...
// Colors for habits
const HABIT_COLORS = [
  "#4ECDC4", "#45B7D1", "#96CEB4", "#FFEAA7", "#FFB6C1", 
  "#DDA0DD", "#20B2AA", "#FF6B6B", "#FFA07A", "#98D8C8",
...
export default function AddHabit() {
  ...
  useEffect(() => {
    if (isEditMode && habitData?.habit) {
      ...
      if (habit.preferredTime) {
        setReminderTime(new Date(habit.preferredTime));
        setEnableReminder(habit.notificationsEnabled);
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditMode && habitId) {
        ...
        await updateHabitMutation.mutateAsync(updateData);
        ...
        Alert.alert("Success", "Habit created successfully!", [
          ...
          { text: "View Calendar", onPress: () => router.push("/(protected)/habit/calendar") },
      ...
      Alert.alert(
        "Error",
        error?.response?.data?.message || "Failed to save habit. Please try again."
      );
    }
  };
  ...
  if (categoriesLoading || (isEditMode && habitLoading)) {
    return (
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <ScrollView className="flex-1 px-4 py-6">
        ...
        {step === 1 && (
          <View>
            {/* Habit Name */}
            <View className="mb-6">
              <Text className="font-semibold text-gray-700 mb-2">Habit Name *</Text>
              <TextInput
              ...
              <View className="flex-row flex-wrap">
                {categories.map((category) => (
                  ...
                  >
                    ...
                    >
                      <Ionicons name={(category.icon || "ellipsis-horizontal-outline") as any} size={14} color="white" />
                    </View>
        ...
        {step === 2 && (
          <View>
            ...
            {/* Frequency */}
            <View className="mb-6">
              <Text className="font-semibold text-gray-700 mb-2">Frequency</Text>
              <View className="flex-row space-x-2">
                {[
                  ...
                  >
                    <Text className={`text-center ${
                      frequency === f.value ? "text-blue-700" : "text-gray-600"
                    }`}>
                      {f.label}
                    </Text>
                  </TouchableOpacity>
            ...
            {/* Repeat Days */}
            <View className="mb-6">
              <Text className="font-semibold text-gray-700 mb-2">Repeat on</Text>
              ...
              <View className="flex-row mt-3 space-x-2">
                <TouchableOpacity
                  ...
                  onPress={() => {
                    const weekdays = { monday: true, tuesday: true, wednesday: true, thursday: true, friday: true };
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    const everyday = DAYS_OF_WEEK.reduce((acc, day) => ({
                      ...acc,
                      [day.key]: true
                    }), {});
            ...
            {Object.values(selectedDays).some(Boolean) && (
              <View className="mb-6 p-4 bg-blue-50 rounded-lg">
                <Text className="font-medium text-blue-800 mb-1">Selected Days:</Text>
        ...
        {step === 3 && (
          <View>
                ...
                {[
                  ...
                  >
                    <View className="items-center">
                      ...
                      />
                      <Text className={priority === p.value ? "text-blue-700" : "text-gray-600"}>
                        {p.label}
                      </Text>
                    </View>
                  </TouchableOpacity>
            ...
            <View className="p-4 bg-gray-50 rounded-xl">
              <Text className="font-semibold text-gray-700 mb-3">Preview</Text>
              <View className="flex-row items-center">
                ...
                <View className="flex-1">
                  <Text className="font-medium text-lg text-gray-800">{habitName}</Text>
                  {selectedCategoryData && (
                    <Text className="text-gray-500">{selectedCategoryData.name}</Text>
                  )}
                  {Object.values(selectedDays).some(Boolean) && (
          ...
          >
            <Text className="font-medium text-white">
              {createHabitMutation.isPending || updateHabitMutation.isPending ? (
                ...
                step === 3 ? (isEditMode ? "Update Habit" : "Create Habit") : "Next"
              )}
            </Text>
          </TouchableOpacity>
        </View>
      ...
      {/* Time Picker Modal */}
      <Modal visible={showTimePicker && enableReminder} animationType="slide" presentationStyle="formSheet">
        <View className="bg-white" style={{ height: 350 }}>
          <View className="flex-row items-center justify-between p-4 border-b border-gray-200">
